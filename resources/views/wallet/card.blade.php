...
<body>
    <div class="card">
        ...
        <h1>{{ __('app.wallet.landing.greeting', ['name' => $member->name]) }} 👋</h1>
        <p class="sub">{{ __('app.wallet.landing.subtitle') }}</p>
        ...
        @foreach (($isAndroid ? ['google', 'apple'] : ['apple', 'google']) as $platform)
            ...
            @elseif ($platform === 'google' && $google)
                <a class="btn btn-google" href="{{ route('wallet.card.google', $token) }}">
                    ...
                    {{ __('app.wallet.landing.google') }}
                </a>
            @endif
        @endforeach

        <p class="hint">
            {{ __('app.wallet.landing.hint') }}
        </p>
    </div>
</body>
</html>
